...
entity sad_bo is
	...
	port(
		clk : in std_logic;
		menor : out std_logic;
		address    : out std_logic_vector(N - 2 downto 0);
		...
		sad_value : out std_logic_vector(O - 1 downto 0)
	);
end entity;
...
architecture structure OF sad_bo is
    signal saidamux1 : std_logic_vector(N - 1 downto 0);
    signal saidareg1 : unsigned(N - 1 downto 0);
    signal soma1res : unsigned(N - 1 downto 0);
    ---------
    signal saidaregA : unsigned(M - 1 downto 0);
    signal saidaregB : unsigned(M - 1 downto 0);
    signal saidasub : unsigned(M - 1 downto 0);
    signal soma2res : unsigned(O downto 0);
    signal saidamux2 : std_logic_vector(O - 1 downto 0);
    signal saidasoma : unsigned(O - 1 downto 0);
    signal resultado : unsigned(O - 1 downto 0);
begin
    -- Bloco 1:
    ...
    address <= std_logic_vector(saidareg1(N-2 downto 0));
    menor <= not saidareg1(N-1);
    som1_inst : entity work.unsigned_adder
        generic map(
            N => N - 1
        )
        port map(
            input_a => resize(saidareg1, N - 1),
            input_b => to_unsigned(1, N - 1),
            sum => soma1res
        );
    ...
    mult2_inst : entity work.mux_2to1
        ...
        port map (
            sel => zsoma,
            in_0 => std_logic_vector(soma2res(O - 1 downto 0)),
            in_1 => (others => '0'),
            y => saidamux2
        );
